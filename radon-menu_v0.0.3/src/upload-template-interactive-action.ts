...
import * as restApi from './tl-rest-api-calls';
...
export async function uploadTemplateInteractiveAction(context: ExtensionContext):Promise<CreateTemplateState> {
    ...
    async function inputTemplateNameForVersion(input: MultiStepInput, state: Partial<CreateTemplateState>): Promise<any> {
        ...
        let csarNamesResponse = await restApi.getCSARsFromName(inputName);
